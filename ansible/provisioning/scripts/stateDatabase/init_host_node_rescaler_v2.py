...
# usage example: init_host_node_rescaler_v2.py host0 cpu [{'container_name': 'host1-cont1', 'host': 'host1', 'cpu_max': 200, 'cpu_min': 50, 'mem_max': 2048, 'mem_min': 1024}, {'container_name': 'host1-cont1'...}] config/config.yml
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) > 4):
        host = sys.argv[1]
        resource = sys.argv[2]
        containers = json.loads(sys.argv[3].replace('\'','"'))
        ...
        for c in containers:
            ...
            if (not_initialized):
                ...
                if (r.ok):
                    ...
                    print("Error initializing " + resource + " value for " + c['container_name'] + " in host " + host)
