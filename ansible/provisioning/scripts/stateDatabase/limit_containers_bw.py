...
def main(hosts_str, config_file):
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) > 2):
        ...
        main(hosts_str, config_file)
    else:
        raise TypeError(f"Error: 2 arguments were expected, but {len(sys.argv) - 1} were received.\n"
                        f"Usage: python {sys.argv[0]} <host list> <yaml file>")
