 ...
 */
// PATH: hadoop-hdfs-project/hadoop-hdfs-client/src/main/java/org/apache/hadoop/hdfs/protocolPB
...
public class ClientNamenodeProtocolTranslatorPB implements
  ...
  // [PATCH]
  final private Configuration clientConf;

  // [PATCH] --- Begin Replication override additions ---
  public static final Logger REPLOG = LoggerFactory.getLogger(ClientNamenodeProtocolTranslatorPB.class);

  private static boolean isExplicitlyConfigured(Configuration conf, String key) {
      String[] sources = conf.getPropertySources(key);
      for (String source : sources) {
          if (!source.contains("hdfs-default.xml") && !source.contains("job.xml")) {
              return true;
          }
      }
      return false;
  ...
  /**
   * Decide the replication to send to the NameNode according to:
   *  - Respect client explicit settings (-Ddfs.replication or hdfs-site.xml).
   *  - If nothing explicit is set,
   *    send -1 so the server-side default is used.
   */
  private static short decideReplication(String path, short requestedByCaller, Configuration conf) {
    boolean clientExplicit = isExplicitlyConfigured(conf, "dfs.replication");
    short effective;

    if (!clientExplicit) {
      // No explicit setting anywhere, caller just propagated the default -> send -1
      effective = (short)-1;
    } else {
      // Respect either the explicit config or the caller's explicit value
      effective = requestedByCaller;
    }
    return effective;
  }
  // --- [PATCH] End Replication override additions ---
  ...
  // [PATCH]
  public ClientNamenodeProtocolTranslatorPB(ClientNamenodeProtocolPB proxy, Configuration config) {
    rpcProxy = proxy;
    clientConf = config;
  }
  ...
  public HdfsFileStatus create(String src, FsPermission masked,
    ...
    // [PATCH] Detect whether the replication was explicitly specified
    final short effectiveReplication;
    if (clientConf != null) {
      effectiveReplication = decideReplication(src, replication, clientConf);
    } else {
    ...
    CreateRequestProto.Builder builder = CreateRequestProto.newBuilder()
        ...
        .setReplication(effectiveReplication)
